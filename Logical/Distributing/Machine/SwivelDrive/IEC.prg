<?xml version="1.0" encoding="utf-8"?>
<?AutomationStudio Version=4.5.2.102?>
<Program SubType="IEC" xmlns="http://br-automation.co.at/AS/Program">
  <Files>
    <File Description="Cyclic code">Cyclic.ld</File>
    <File Description="Initialization code">Init.ld</File>
    <File Description="Exit code">Exit.ld</File>
    <File Description="Local data types" Private="true">Types.typ</File>
    <File Description="Local variables" Private="true">Variables.var</File>
    <File>StateRunCycle.ld</File>
    <File>StateWait.ld</File>
    <File>StateActivateVacuum.ld</File>
    <File>StateDeactivateVacuum.ld</File>
    <File>StateError.ld</File>
    <File>StateHome.ld</File>
    <File>StateMoveToConveyor.ld</File>
    <File>StateMoveToMagazine.ld</File>
  </Files>
</Program>